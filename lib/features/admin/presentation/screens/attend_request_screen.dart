        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              ...
              if (confirm == true) {
                await _service.deleteTripRequestCascade(widget.request.id!);
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _isAssigning ? 'Desvinculando...' : 'Desvincular Conductor',
                ),
                style: ElevatedButton.styleFrom(
  ...
  Widget _buildDriverCard(dynamic driver, {required bool isAccepted}) {
    ...
    if (isAccepted) {
      ...
      data = {
        ...
        'phoneNumber':
            raw['phone_number'] ?? raw['phoneNumber'] ?? raw['phone'] ?? '',
        'licenseNumber': raw['license_number'] ?? raw['licenseNumber'] ?? '',
        'vehicleCapacity':
            raw['vehicle_capacity'] ?? raw['vehicleCapacity'] ?? 0,
        'routes': raw['routes'] ?? <String>[],
        'profilePhoto':
            raw['photo_url'] ?? raw['photoUrl'] ?? raw['photo'] ?? '',
        'vehiclePhotos':
            raw['vehicle_photo_url'] != null
                ? [raw['vehicle_photo_url'].toString()]
                : (raw['vehiclePhotos'] ?? <String>[]),
      };
    } else {
      ...
      data = {
        ...
        'vehiclePhotos':
            d.vehiclePhotoUrl.isNotEmpty ? [d.vehiclePhotoUrl] : <String>[],
      };
    }
    ...
    final String? profile =
        (data['profilePhoto'] as String?)?.isEmpty == true
            ? null
  ...
  Future<void> _assignDriver(
    ...
    final bool? confirmed = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            title: Text(
              isRejection ? 'Confirmar Rechazo' : 'Confirmar Asignación',
            ),
            content: Text(
              isRejection
