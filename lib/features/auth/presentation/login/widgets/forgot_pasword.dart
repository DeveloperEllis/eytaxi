...
Future<void> showForgotPasswordDialog(BuildContext context, {String? prefillEmail}) async {
  return showDialog(
    context: context,
...
class ForgotPasswordButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextButton(
      onPressed: () => showForgotPasswordDialog(context, prefillEmail: prefillEmail),
...
class _ForgotPasswordDialogState extends State<_ForgotPasswordDialog> {
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.lock_reset, color: AppColors.primary, size: 24),
                ),
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      ...
      // Mensaje de éxito
      LogsMessages.showSuccess(context, 'Te enviamos un enlace para restablecer tu contraseña. Revisa tu correo.');
    } on AuthException catch (e) {
      final msg = e.message.toLowerCase();
      String message = 'No pudimos procesar tu solicitud.';
      
      if (msg.contains('failed to fetch') || msg.contains('network') || msg.contains('connection')) {
        message = 'No se pudo conectar al servidor. Verifica tu conexión a internet.';
      } else if (msg.contains('user') && msg.contains('not') && msg.contains('found')) {
      ...
      LogsMessages.showError(context, 'Ocurrió un error inesperado. Intenta nuevamente.');
    } finally {
